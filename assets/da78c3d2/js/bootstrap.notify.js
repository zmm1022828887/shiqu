...
(function ($) {
  var Notification = function (options) {
    ...
    // Element collection
    if ($('.notifications').length > 0 && $('.notifications').hasClass(this.options.position)) {
      this.$element = $('.notifications').filter('.'+this.options.position);
    } else {
      this.$element = $('<div class="notifications"></div>');
      this.$element.addClass(this.options.position);
      this.$element.appendTo(this.options.containner);
    }
      ...
      if (typeof this.options.message === 'string')
        ...
        if (this.options.message.html)
          ...
          if (this.options.message.icon)            
            this.$note.append($('<i class="' + this.options.message.icon + '"></i>'));
