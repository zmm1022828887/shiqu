...
(function() {
	...
	CKEDITOR.plugins.add( 'clipboard', {
		...
		icons: 'copy,copy-rtl,cut,cut-rtl,paste,paste-rtl,remove', // %REMOVE_LINE_CORE%
	...
	function initClipboard( editor ) {
		...
		function addButtonsCommands() {
			...
			addButtonCommand( 'Remove', 'remove', createDeleteCmd( 'remove' ), 40, 16 );
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element, selection ) {
					...
					return {
						...
						remove: stateFromNamedCommand( 'Remove')
					};
				});
			}
		}
		...
		function createDeleteCmd( type ) {
			return {
				type: type,
				canUndo: type == 'remove', // We can't undo copy to clipboard.
				startDisabled: true,
				exec: function( data ) {
					// Attempts to execute the Cut and Copy operations.
					function tryToDelete( type ) {
						if ( CKEDITOR.env.ie )
							return execIECommand( type );
					...
					var success = tryToDelete( this.type );
