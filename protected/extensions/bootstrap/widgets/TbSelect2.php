...
class TbSelect2 extends CInputWidget
{
  ...
  /**
   * @var array @param data for generating the ajax (value=>display)
   */
  ...
  public function registerClientScript($id)
  {
    ...
    Yii::app()->bootstrap->registerAssetJs('select2.min.js');
    Yii::app()->bootstrap->registerAssetJs('select2_locale_zh-CN.js');
    
    if(!empty($this->ajaxOptions)){
        $this->options['ajax'] = array(
            'url' => $this->ajaxOptions['url'],
            'dataType' => 'json',
            'type' => $this->ajaxOptions['type'] ? $this->ajaxOptions['type'] : "GET",
            'data' => 'js:function (term, page) {return {'.($this->ajaxOptions['field'] ? $this->ajaxOptions['field'] : 'q').': term};}',
