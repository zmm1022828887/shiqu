...
class TbInputHorizontal extends TbInput {
    ...
    protected function selector() {
        ...
        if(isset($this->htmlOptions['single']))
            ...
            $single = false;
        $btnOpt = $this->htmlOptions['btnOpt'];
        unset($this->htmlOptions['type']);
        unset($this->htmlOptions['single']);
        unset($this->htmlOptions['btnOpt']);

        switch (strtolower($type)) {
            case 'user':
                $view = $single == false ?  'selectuser' : 'selectsingleuser';
                $class = 'User';
                break;
            case 'dept':
                $view = $single == false ?  'selectdept' : 'selectsingledept';
                $class = 'Department';
                break;
            case 'role':
                $view = 'selectrole';
        ...
        // 控件ID
        $hidId = get_class($this->model) . '_' . $this->attribute;
        $textId = $hidId . '_name';
        $attr = $this->attribute;
        // 配置属性
        $this->htmlOptions['id'] = $textId;
        ...
        // 文本域
        if ($class !== null && class_exists(ucwords($class)) && is_callable(array(ucwords($class), 'getNameById'))) {
            $value = call_user_func(ucwords($class) . '::getNameById', $this->model->$attr);
        } else {
            $value = $this->model->$attr;
        }
        if(!$value)
            $value = '';
        echo CHtml::tag('textarea', $this->htmlOptions, $value);

        // 按钮
        $this->widget('bootstrap.widgets.TbButton', array(
            'label' => ($btnOpt['addlabel'] ? $btnOpt['addlabel'] : '添加'),
            'size' => ($btnOpt['size'] ? $btnOpt['size'] : 'mini'),
            'htmlOptions' => array(
                'class' => $btnOpt['class'],
                'style' => ($btnOpt['style'] ? $btnOpt['style'] : 'vertical-align:bottom;'),
                'data-toggle' => 'modal',
                'onclick' => 'vk_' . $view . '("' . $hidId . '","' . $textId . '","' . $view . '")',
            ),
        ));
        $this->widget('bootstrap.widgets.TbButton', array(
            'label' => ($btnOpt['clearlabel'] ? $btnOpt['clearlabel'] : '删除'),
            'size' => ($btnOpt['size'] ? $btnOpt['size'] : 'mini'),
            'htmlOptions' => array(
                'class' => $btnOpt['class'],
                'style' => ($btnOpt['style'] ? $btnOpt['style'] : 'vertical-align:bottom;'),
                'onclick' => 'vk_del_' . $view . '("' . $hidId . '","' . $textId . '")',
            ),
        ));
        echo $this->getError() . $this->getHint();
