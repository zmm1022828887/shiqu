...
class TbGridView extends CGridView {
    ...
    /**
     * @var string the CSS class name for the pager container. Defaults to 'toolbar'.
     */
    ...
    public $toolbar = array('class' => 'bootstrap.widgets.TbButtonGroup');
    ...
    public function renderToolbar() {
        ...
        if ($count > 0) {
            for ($i = 0; $i < $count; $i++) {
                if (array_key_exists('label', $toolbar[$i])) {
                    $this->widget($TbButton, array(
                        'label' => $toolbar[$i]['label'],
                        'type' => $toolbar[$i]['type'],
                        'size' => $toolbar[$i]['size'],
                        'icon' => $toolbar[$i]['icon'],
                        'htmlOptions' => $toolbar[$i]['htmlOptions']));
                } else {
                    $this->widget($TbButtonGroup, array(
                        'size' => $toolbar[$i]['size'],
                        'type' => $toolbar[$i]['type'],
                        'buttons' => $toolbar[$i]['buttons']));
                }
            }
        }
        echo '</div>';
