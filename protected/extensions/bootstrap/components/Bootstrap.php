...
class Bootstrap extends CApplicationComponent
{
	 ...
	 */
	public $forceCopyAssets = false; //YII_DEBUG;//开启DEBUG性能影响很大
    ...
    /**
	 * @var boolean whether to use IcoMoon.
	 */
	public $useIcoMoon = true;

	/**
	 * @var bool 是否使用bsie包.
	 */
	public $useBsie = false;
	...
	public function init()
	{
		...
		if($this->useBsie !== false){
			$this->registerBsieCss();
			$this->registerBsieJs();
		}

		if($this->useIcoMoon !== false)
			$this->registerIcoMoonCss();
	...
	/**
	 * Registers the bsie CSS.
	 */
	public function registerBsieCss()
	{
		if((strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 6.') !== FALSE) || (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 7.') !== FALSE)) 
			$this->registerAssetCss('bootstrap-ie6' . (!YII_DEBUG ? '.min' : '') . '.css');
		if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 7.') !== FALSE) 
			$this->registerAssetCss('ie.css');
	}

	/**
	 * Registers the bsie js.
	 */
	public function registerBsieJs()
	{
		if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 6.') !== FALSE)
		{
			$this->registerAssetJs('bootstrap-ie.js',CClientScript::POS_HEAD);
			$this->registerAssetJs('icomoon-lte-ie7.js');
		}
	}
	 ...
	 * @since 0.9.11
	 */
	public function registerIcoMoonCss()
	{
		$this->registerAssetCss('icomoon.css');
	}
